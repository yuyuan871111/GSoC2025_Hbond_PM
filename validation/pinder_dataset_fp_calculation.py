...
df = pd.read_csv("./data/pinder_val_systems.csv")
...
for idx in tqdm(df.index):
    ...
    if (
        pathlib.Path(f"./pinder_val/explicit/fp_{idx}.pkl").exists()
        and pathlib.Path(f"./pinder_val/implicit/fp_{idx}.pkl").exists()
    ):
        continue
    ...
    try:
        ...
        if not pathlib.Path(f"./pinder_val/explicit/fp_{idx}.pkl").exists():
            fp = plf.Fingerprint(["HBDonor", "HBAcceptor"], count=True)
            fp.run_from_iterable([ligand], protein_mol, progress=False)
            fp.to_pickle(f"./pinder_val/explicit/fp_{idx}.pkl")
        ...
        if not pathlib.Path(f"./pinder_val/implicit/fp_{idx}.pkl").exists():
            fp = plf.Fingerprint(
            ...
            fp.to_pickle(f"./pinder_val/implicit/fp_{idx}.pkl")
